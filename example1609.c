 ...
 *  implement the actual OBU / RSU functions - @ref mod_rsa_rx, @ref
 *  mod_tim_rx and @ref mod_evw_tx for OBE and @ref mod_rsa_tx,
...
typedef struct App
{
  ...
  char pAppConfigFilename[255];
  /// Example stack configuration file name
  char pStackConfigFilename[255];
  /// Extra configuration file name
  char pExtraConfigFilename[255];
...
int main (int Argc, char ** ppArgv)
{
  ...
  for (ArgIndex = 0; ArgIndex < Argc; ArgIndex++)
  {
    if (strcmp(ppArgv[ArgIndex], OPTS_TAG_APP_CFG_FILENAME) == 0)
    {
      if ((ArgIndex + 1) < Argc)
      {
        strcpy(pApp->pAppConfigFilename, ppArgv[ArgIndex + 1]);
  ...
  for (ArgIndex = 0; ArgIndex < Argc; ArgIndex++)
  {
    if (strcmp(ppArgv[ArgIndex], OPTS_TAG_STACK_CFG_FILENAME) == 0)
    {
      if ((ArgIndex + 1) < Argc)
      {
        strcpy(pApp->pStackConfigFilename, ppArgv[ArgIndex + 1]);
  ...
  for (ArgIndex = 0; ArgIndex < Argc; ArgIndex++)
  {
    if (strcmp(ppArgv[ArgIndex], OPTS_TAG_EXTRA_CFG_FILENAME) == 0)
    {
      if ((ArgIndex + 1) < Argc)
      {
        strcpy(pApp->pExtraConfigFilename, ppArgv[ArgIndex + 1]);
  ...
  if (Stat != 0)
    goto Exit;
  // Init thread attibute data and start signal handler
  ...
  Stat = pthread_attr_setstacksize(&Attr, APP_THREAD_STACK_SIZE);
  if (Stat != 0)
  {
    d_printf(D_ERR, NULL, "pthread_attr_setstacksize failed [%d] %s\n",
             errno, strerror(errno));
    goto Exit;
  }
  Stat = pthread_attr_setschedpolicy(&Attr, SCHED_FIFO);
  if (Stat != 0)
  {
...
#ifndef __aarch64__
  Stat = pthread_attr_setinheritsched(&Attr, PTHREAD_EXPLICIT_SCHED);
  if (Stat)
  {
...
#endif
  SchParam.sched_priority = THREAD_NORMAL_PRIORITY;
  Stat = pthread_attr_setschedparam(&Attr, &SchParam);
  if (Stat)
  {
    d_printf(D_ERR, NULL, "pthread_attr_setschedparam [%d] %s\n",
             errno, strerror(errno));
    goto AppInitError;
  }
  if (pApp->Cfg.RSATx)
  ...
  // Dump all the stack and plugin configuration into conf file
  const tIniFile *pIni = ConfigGetIni();
  d_assert(pIni);
  if (Log_GetLoggingEnabled() && pIni->Cohda_LogCaptureConf)
  {
    char Path[LOG_PATH_MAX] = {0};
    Log_GetPath("conf", Path);
    Conf2_DumpTo(Path, false, false);
  }
  ConfigReleaseIni(pIni);
  // Running
...
/**
 * @brief Intialise stack components
...
static int Stack_Init (tApp *pApp, bool ExtraConfigFilename, bool ConfAllow)
{
  ...
  // re-read the application configuration file and stash it in the log direcrory
  {
    config_t Config;
    config_init(&Config);
  ...
  #if defined(V2XATK)
  {
    ll_init = NULL;
    ll_get = NULL;
    // (pIni->ProtocolMode == CONF_PROTOCOL_MODE_1609)
    {
    ...
    {
      d_printf(D_ERR, NULL, "ll_init() or ll_get() unavailible\n");
      goto Exit;
    }
  }
  #endif // V2XATK
...
/**
 * @brief Periodially output statistics of various modules
...
static void App_Exit (void)
{
  ...
  if (pApp->Hdl.pCANVState != NULL)
    CANVState_PrintStats(pApp->Hdl.pCANVState);
  // Close RSATx / RSARx / RawTx / EVWTx / SPATTx / MAPTx TIMGenericRx / TIMRx /
  //       OBERx / CANVState (stops threads, free memory)
  if (pApp->Hdl.pRSATx != NULL)
  {
